...
export async function getIndustryInsights() {
    ...
    `;
        ...
        // Check if insights need updating (gracefully handle AI failures)
        const currentTime = new Date();
        if (new Date(industryInsight.nextupdate) <= currentTime) {
            try {
                console.log(`Attempting to update insights for ${industryInsight.industry}`);
                ...
                console.log(`Successfully updated insights for ${industryInsight.industry}`);
            } catch (updateError) {
                console.warn(`Failed to update insights for ${industryInsight.industry}, using existing data:`, updateError.message);
                
                // Extend next update time to avoid constant retries (2 hours from now)
                try {
                    await db.query(
                        'UPDATE industry_insights SET nextupdate = CURRENT_TIMESTAMP + INTERVAL \'2 hours\' WHERE id = $1',
                        [industryInsight.id]
                    );
                } catch (extendError) {
                    console.warn('Could not extend next update time:', extendError.message);
                }
                
                // Add flag to indicate data might be stale
                industryInsight.isStale = true;
                industryInsight.lastUpdateError = updateError.message.includes('AI_SERVICE_UNAVAILABLE') ? 'AI service temporarily unavailable' : 'Update failed';
            }
        }
