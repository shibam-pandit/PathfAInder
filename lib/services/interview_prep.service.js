                ...
                [
                    {
                        ...
                        "explanation": "one line explanation of the correct answer",
...
export const getInprovementTip = async(data) => {
    try {
        const userId = await getUserId();
        if (!userId) {
            throw new Error("Unauthorized");
        ...
        for (let i = 0; i < data.questions.length; i++) {
            ...
            const correctAnswer = data.questions[i].correct_answer;

            if (userAnswer !== correctAnswer) {
                wrongAnswers.push({
                    question: question,
                    userAnswer: data.questions[i].options[userAnswer],
                    correctAnswer: data.questions[i].options[correctAnswer],
                });
            }
...
export const saveQuizResult = async (quizData) => {
    try {
        ...
        const { questions, quizscore, improvementTip, category } = quizData;
